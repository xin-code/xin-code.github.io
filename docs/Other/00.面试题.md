...
- 相对而言，Post请求更安全，get请求不安全
  - get请求会直接把请求参数放在URL上，post则不会
  - post请求不会放在浏览器历史和服务器日志中，get请求会
  - 但是从`数据传输`的角度来看，post和get都不安全，因为http协议本身是明文传输，抓包的话解析get和post都是一样的
- get请求可以缓存，post则不能缓存
- get有请求长度限制，post没有
  - get请求本身是没有长度限制的，但是URL地址是有的，不同的浏览器对get请求的长度限制也不一样，在兼容IE的情况下，请求的长度只能有2083个字符
- get请求只能传输字符串，post则可以传输多种类型
- get请求的入参在URL上，post请求入参在Request Body上
- post可能产生两个数据包，get只会发送一个
  - post 第一个 tcp发送 header 确认服务器可以相应并且具备接受数据的能力，响应100
  - 第二个tcp包在第一个请求成功(100)才会发送，包含data，如果请求失败，则后续data不会发送
- 刷新和回退get请求无害，post数据会被重新提交
...
- Vue2中 v-for优先
- Vue3中v-if优先
- :triangular_flag_on_post:不推荐一起使用